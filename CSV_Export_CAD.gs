// saves CSV file in google drive -- REQUIRED for upload to QBO
function saveAsCSVCAD() {
  var today = new Date();
  ...
  var sheet = ss.getSheetByName("CAD-Transactions");
  var cell = sheet.getRange(2, 9).getValue();
  Logger.log(cell);
  if(cell == "YES"){
    
    
    var filename = "CAD_Transactions_" + today + ".csv"; // CSV file name
    var Cfilename = "CAD_Transactions.csv"; // Constant CSV file name
  ...
  if(cell == "NO"){ 
    ...
    var Bfilename = "CAD_Transactions.csv"; // Constant CSV file name
    var folder = ""; // Folder ID
